...
Shader "Mock"
{
	Properties
	{
		...
		_LimRange("LimRange", Float) = 0.63
		[ASEEnd]_LimPower("LimPower", Float) = 0.96
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float4 ShadowCoords = float4( 0, 0, 0, 0 );
					...
					#endif
				#endif
				float2 uv_TextureSample011 = IN.ase_texcoord3.xy;
				float4 tex2DNode11 = tex2D( _TextureSample0, uv_TextureSample011 );
				float3 normalizeResult53 = normalize( ( _WorldSpaceCameraPos - WorldPosition ) );
				float3 ase_worldNormal = IN.ase_texcoord4.xyz;
				float dotResult28 = dot( normalizeResult53 , ase_worldNormal );
				float temp_output_1_0_g3 = _LimRange;
				float4 lerpResult45 = lerp( tex2DNode11 , ( tex2DNode11 * _Color0 ) , ( min( ( ( ( ( 1.0 - max( 0.0 , saturate( dotResult28 ) ) ) - temp_output_1_0_g3 ) / ( 1.0 - temp_output_1_0_g3 ) ) * _LimPower ) , 1.0 ) * _LimPower ));
				
				float3 BakedAlbedo = 0;
				float3 BakedEmission = 0;
				float3 Color = lerpResult45.rgb;
				float Alpha = 1;
...
ASEEND*/
//CHKSM=F9DB39B29644DA1C5004F8049E0365A5D42A982D